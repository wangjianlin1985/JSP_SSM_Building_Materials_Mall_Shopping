			...
			<div class="widget">
				...
				<table cellpadding="0" cellspacing="0" width="100%" class="sTable">
					...
					<c:forEach items="${jiancaiList}" var="jiancai">
						<tr align="center">
							<td>${jiancai.jiancainame}</td>
							<td>${jiancai.catename}</td>
							<td>${jiancai.price}</td>
							<td>${jiancai.recommend}</td>
							<td>${jiancai.thestart}</td>
							<td>${jiancai.theend}</td>
							<td>${jiancai.hits}</td>
							<td>${jiancai.sellnum}</td>
							<td><a href="jiancai/getJiancaiById.action?id=${jiancai.jiancaiid}">编辑</a>&nbsp;&nbsp;<a
								href="jiancai/deleteJiancai.action?id=${jiancai.jiancaiid}" onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a></td>
						</tr>
					</c:forEach>
				</table>
