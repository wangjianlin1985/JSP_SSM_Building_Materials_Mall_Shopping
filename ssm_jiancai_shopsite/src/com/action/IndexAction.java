...
public class IndexAction extends BaseAction
{
    ...
    public String addcart() {
        ...
        cart.setJiancaiid(this.getRequest().getParameter("goodsid"));
    ...
    public String preCheckout() {
        ...
        final List<Cart> cartList = this.cartService.getCartByCond(cart);
        if (cartList.size() == 0) {
            this.getRequest().setAttribute("message", (Object)"\u8bf7\u9009\u8d2d\u5efa\u6750\u5546\u54c1");
            return "redirect:/index/cart.action";
        }
    ...
    public String checkout() {
        ...
        for (final Cart cart2 : cartList) {
            ...
            details.setJiancaiid(cart2.getJiancaiid());
            ...
            final Jiancai goods = this.jiancaiService.getJiancaiById(cart2.getJiancaiid());
    ...
    public String cate(final String id, String number) {
        this.front();
        final Jiancai goods = new Jiancai();
        goods.setCateid(id);
        final List<Jiancai> flimList = new ArrayList<Jiancai>();
        final List<Jiancai> tempList = this.jiancaiService.getJiancaiByCond(goods);
        int maxPage;
        final int pageNumber = maxPage = tempList.size();
        if (maxPage % 12 == 0) {
            maxPage /= 12;
        ...
        for (int i = start; i < over; ++i) {
            final Jiancai x = tempList.get(i);
            flimList.add(x);
        }
        String html = "";
        final StringBuffer buffer = new StringBuffer();
    ...
    public String getHost(String number) {
        ...
        goods.setRecommend("\u662f");
        final List<Jiancai> flimList = new ArrayList<Jiancai>();
        final List<Jiancai> tempList = this.jiancaiService.getJiancaiByHot();
        int maxPage;
        final int pageNumber = maxPage = tempList.size();
        if (maxPage % 12 == 0) {
            maxPage /= 12;
    ...
    public String all(String number) {
        this.front();
        final List<Jiancai> flimList = new ArrayList<Jiancai>();
        final List<Jiancai> tempList = this.jiancaiService.getAllJiancai();
        int maxPage;
        final int pageNumber = maxPage = tempList.size();
        if (maxPage % 12 == 0) {
            maxPage /= 12;
    ...
    public String query(final String name) {
        ...
        final List<Jiancai> flimList = this.jiancaiService.getJiancaiByLike(goods);
        this.getRequest().setAttribute("flimList", (Object)flimList);
        return "users/list";
    }
    ...
    public String detail(final String id) {
        this.front();
        final Jiancai goods = this.jiancaiService.getJiancaiById(id);
